...
> ⚠️ **文档说明**：本 README 同时包含**目标设计蓝图**和**当前实现状态**两部分内容。
> 带有 🟢 **已实现** / 🟡 **部分实现** / 🔵 **设计蓝图（尚未实现）** 标记的章节请以标记为准。
> 未标记的架构、命令和目录树多为**设计蓝图**，描述的是项目的目标形态，而非当前可运行的功能。

## ⚠️ 项目状态（Project Status）

**当前阶段：早期原型 / 脚手架（Early Prototype）。** 本项目已经搭好了清晰的工程骨架，但**尚未打通"控制台 → Qubes → 云平台"的真实链路**。请在使用前了解以下现状。

### 现在能做什么

- 🟢 **管理控制台（本地 CRUD）**：Go(Gin) 后端 + Svelte 5 前端，可增删改查 Zone / Qube / 凭据 / 基础设施等记录。分层清晰，有单元测试与 CI。
- 🟢 **凭据加密存储**：云凭据使用 AES-256-GCM 加密后存入本地 SQLite（`credential_repository.go`）。
- 🟢 **Salt WireGuard 网关状态**：`salt/qubes-air/sys-remote/` 下的 WireGuard / 防火墙 / 网关 states 是**真实可用**的——若指向真实的 minion，能配置出一个 WireGuard 网关。
- 🟢 **辅助脚本**：`crypto/scripts/` 下的密钥生成（WireGuard + age）、SOPS 加密脚本可独立使用。
- 🟢 **qrexec 客户端代码**：`console/backend/internal/qrexec/client.go` 实现了对 `qrexec-client-vm` 的调用。

### 现在还不能做什么（重要）

- 🔴 **控制台不会真正创建/启动任何 Qube 或云 VM。** `Qube.Start()`、`Infra.Connect()` 等操作目前只是**修改 SQLite 里的状态字段**，不调用 qrexec、不调用 Terraform、不启动任何真实资源。
- 🔴 **qrexec 客户端是"死代码"。** 虽然实现了，但**没有任何 service / handler 调用它**——控制台与真实 Qubes 之间尚无活动代码路径。
- 🔴 **Terraform 不创建任何真实资源。** 目前全部 module 中唯一的真实 `resource` 是 `random_id`；所有 provider/VM 输出都是 `""` 或 `"pending"` 占位符。**不会建出任何一台 VM。**
- 🔴 **控制台与 Terraform / Salt / Ansible 四层互不连通**，没有编排引擎把控制台的数据库状态"落地"成真实操作。
- 🔴 **API 无认证。** 所有 `/api/v1` 路由（含读写云凭据）目前无任何身份验证（详见下方"已知问题"）。
- 🔴 **监控 / 账单是占位数据。** 监控接口返回的是**控制台进程自身**的 Go 运行时内存；账单接口返回硬编码 `0.00`。

### 已知问题（Known Issues，按优先级）

1. **API 完全无认证**，且默认 CORS 为 `*` 同时开启 `Allow-Credentials: true`（危险组合）。修复前不要暴露到可信网络之外。
2. **默认加密密钥硬编码在源码中**（`config.go` / `main.go` 中的 `qubes-air-dev-encryption-key32!!`），未配置有效密钥时会**静默回退**到该公开密钥——此时凭据加密形同虚设。请务必通过配置提供自己的 32 字节密钥。
3. 本 README 与 `docs/` 中大量描述超前于实现（见上文标注）。

### 路线图（诚实版）

短期优先级：**(a)** 补齐 API 认证 + 修复弱密钥/CORS → **(b)** 打通一条真实端到端链路（建议先 Proxmox）→ **(c)** 对齐官方 R4.3 的 **RemoteVM** 机制。详见文末[路线图](#路线图)。
...
> 🔵 **对齐待办**：本项目当前的 `sys-remote + WireGuard 网关 + 自定义 qrexec 服务` 方案，是在官方 RemoteVM 落地**之前**的自造设计。方向一致（都走 qrexec），但**尚未对齐 R4.3 的 RemoteVM 原语**（`RemoteVM` 类 + `relayvm` / `transport_rpc` / `remote_name` 属性 + Relay 上的 `qubesair.SSHProxy` transport）。
>
...
> 🔵 **本章节为设计蓝图（目标架构），描述项目希望达到的形态，尚未在代码中实现。** 当前真实实现请见文首「项目状态」。
...
> 🟡 **实现状态**：后端与前端骨架**已实现**（CRUD + 加密存储）；下表中 gRPC、OAuth2/mTLS 认证为 🔵 **蓝图，尚未实现**。当前 API **无认证**。
...
| 认证 | OAuth2 / mTLS | 🔴 **未实现** | 当前**无任何认证**，为已知高优先级问题 |
...
|  +--------------------------------------------+  |
|  |  系统集成层  🔵 蓝图（尚未连通）            |  |
|  |  - Terraform CLI 调用   （未实现）         |  |
|  |  - Salt API 集成        （未实现）         |  |
|  |  - qrexec 服务调用      （代码存在但未接入）|  |
|  +--------------------------------------------+  |
+--------------------------------------------------+
```

> 🔴 上图"系统集成层"是**目标形态**。当前后端**不调用** Terraform / Salt，qrexec 客户端代码存在但**未被任何 service 调用**。这是从"模拟"走向"MVP"最关键的缺失环节。
...
> 🔵 **本章节（威胁模型、隔离多元化、HSM/KMS 集成、密钥层级、磁盘加密等）为目标安全设计蓝图，绝大部分尚未在代码中实现。** 其中的 Terraform `aws_kms_key` / `google_kms_crypto_key`、`disk-encryption.sls`、Split GPG 工作流等均为示例，仓库中并无对应实现。
>
> **当前真实的安全现状**请以文首「已知问题」为准：API 无认证、默认加密密钥硬编码、CORS 配置不安全。已实现的正面项：凭据 AES-256-GCM 加密存储、SOPS/age 加密脚本、Salt 防火墙 states。
...
> 以下是**当前仓库的真实结构**（截至最新提交）。此前版本的结构树包含大量尚未创建的文件/目录，已更正。带 🔵 的条目为文末[路线图](#路线图)中规划的目标结构。

```
qubes-air/
|-- terraform/                       # 🟡 骨架：目前唯一真实 resource 是 random_id，不创建 VM
|   |-- main.tf                      #    顶层：按开关实例化各 Zone module
|   |-- variables.tf
|   |-- outputs.tf
|   |-- modules/
...
|   |-- environments/                #    (空)
|-- salt/                            # 🟢 sys-remote 网关 states 真实可用
|   |-- qubes-air/
|   |   |-- top.sls
|   |   |-- common/base.sls
|   |   |-- sys-remote/
...
|-- readme.md
|
|-- (🔵 规划中：terraform/modules/{networking,security,kms,remote-qube-*}、
...
> 🔵 **以下命令为设计蓝图，当前无法端到端运行。** 本节引用的多个脚本（如 `create-sys-remote.sh`、`zone-manage.sh`、`qubes-air-ctl`）**尚未实现**；`terraform apply` 也不会创建真实 VM。此处保留是为了说明**目标使用方式**。
>
...
> 🔵 **本章节为设计蓝图。** 下列 `qubes-air-ctl`、`qubesctl --targets` 等命令依赖尚未实现的控制脚本与真实集成，**当前不可用**。
...
### Phase 0 - 修复关键问题（当前最高优先级）🔴

在推进任何新功能前，先修掉会误导用户或危及安全的问题：

- [ ] 为 `/api/v1` 添加认证（token / mTLS），修复 CORS `*` + credentials 组合
- [ ] 无有效加密密钥时**拒绝启动**，移除硬编码默认密钥的静默回退
- [x] README 诚实化：区分"已实现"与"设计蓝图"（本次更新）
- [ ] 移除或明确标注占位式监控/账单数据
