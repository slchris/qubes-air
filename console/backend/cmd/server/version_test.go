...
// between the two is silent in both directions: `make build-backend` succeeds
// and ships a binary reporting "unknown" for the field that stopped matching.
// Nothing else in the tree asserts that recipe, so this is the only test that
// fails when CONSOLE_STAMP drifts.
//
...
func TestMakefileStampsTheBinaryItBuilds(t *testing.T) {
	...
	if out, err := exec.Command("make", "-C", repoRoot, "build-backend").CombinedOutput(); err != nil {
		t.Fatalf("make build-backend: %v\n%s", err, out)
	}

	bin := filepath.Join(repoRoot, "console", "backend", "bin", "qubes-air-console")
	line := runVersion(t, bin)

	if got, want := stampField(t, line, "version"), gitOutput(t, repoRoot, "describe", "--tags", "--always", "--dirty"); got != want {
		t.Errorf("make build-backend: version = %q, want this tree's `git describe --tags --always --dirty` = %q", got, want)
	}
	if got, want := stampField(t, line, "revision"), gitOutput(t, repoRoot, "rev-parse", "HEAD"); got != want {
		t.Errorf("make build-backend: revision = %q, want `git rev-parse HEAD` = %q", got, want)
	}
	if got := stampField(t, line, "build_time"); !strings.HasSuffix(got, "Z") {
		t.Errorf("make build-backend: build_time = %q, want the UTC `date -u +%%Y-%%m-%%dT%%H:%%M:%%SZ` form", got)
	} else if _, err := time.Parse(time.RFC3339, got); err != nil {
		t.Errorf("make build-backend: build_time = %q is not an RFC 3339 timestamp: %v", got, err)
	}
	if got := stampField(t, line, "tree"); got != string(buildinfo.TreeClean) && got != string(buildinfo.TreeDirty) {
		t.Errorf("make build-backend: tree = %q, want %q or %q", got, buildinfo.TreeClean, buildinfo.TreeDirty)
	}
}
