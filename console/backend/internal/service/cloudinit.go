...
func RenderAgentUserData(remoteName string, id AgentIdentityDoc, listen string, pkg AgentPackage, encryptData bool) (string, error) {
	...
	writeUserDataHeader(&b)
	writeHostnameAndPackages(&b, remoteName, pkg, encryptData)
...
// configuration rather than a script, plus the comment block explaining what
// the document is for. These bytes are part of the delivered contract; they
// parallel docs/bootstrap-design.md section 9.
func writeUserDataHeader(b *strings.Builder) {
...
// package list.
//
// The guest must know its own name. Without this every qube in the fleet comes
// up as "localhost" with an EMPTY /etc/hostname — verified on real hardware —
// so nothing in a log line says which machine produced it, and fleet-wide
// troubleshooting has to correlate by IP. Empty is worse than wrong: tools that
// read /etc/hostname directly get nothing at all.
//
// It matters more here than on an ordinary VM because this name already exists
// in three places that must agree — the Proxmox VM name, the agent
...
// hostname with no matching entry makes anything resolving its own name wait
// for a DNS timeout first.
func writeHostnameAndPackages(b *strings.Builder, remoteName string, pkg AgentPackage, encryptData bool) {
...
// writeIdentityFiles emits the write_files section.
//
// The CA is public — it is what the agent verifies the console WITH, not a
// secret. The token is not public, but it authorizes exactly one certificate,
// for exactly one qube, once, and only until it expires. The token is written
// mode 0600 and the CA 0644: a world-readable token lets any local account on
// the guest claim this qube's identity before the agent does. The blast radius
// is one qube and one boot rather than a 90-day credential, which is the trade
// the token design bought.
//
// There is deliberately no agent.pem and no agent-key.pem here. The agent
// generates its own key at first boot and hands out only a CSR, so the private
// key never exists anywhere but the machine it authenticates. That closes the
// hole this document used to be: on Proxmox, cloud-init data is readable
// through the API by anyone holding VM.Config.Cloudinit, which made that
// permission equivalent to holding every agent identity ever delivered.
func writeIdentityFiles(b *strings.Builder, id AgentIdentityDoc, remoteName, listen string, pkg AgentPackage, encryptData bool) {
...
// writeRuncmd emits the runcmd section that starts the agent only after the
// files exist. cloud-init runs runcmd after write_files, so the ordering is
// guaranteed rather than hoped for.
func writeRuncmd(b *strings.Builder, encryptData bool) {
