...
func issuerFixture(t *testing.T) (*CertIssuer, *repository.AgentCertRepository, *memCredStore) {
	...
	return NewCertIssuer(store, certs, t.TempDir(), "0.0.0.0:8443", testAgentPackage()), certs, store
}
...
func TestCASurvivesRestart(t *testing.T) {
	...
	first, err := NewCertIssuer(store, certs, t.TempDir(), "0.0.0.0:8443", testAgentPackage()).IssueFor(ctx, testQube("alpha"))
	require.NoError(t, err)

	// A completely new issuer, as after a process restart.
	second, err := NewCertIssuer(store, certs, t.TempDir(), "0.0.0.0:8443", testAgentPackage()).IssueFor(ctx, testQube("beta"))
	require.NoError(t, err)
...
func TestHalfPresentCARefuses(t *testing.T) {
	...
	fresh := NewCertIssuer(store, repository.NewAgentCertRepository(certTestDB(t)), t.TempDir(), "0.0.0.0:8443", testAgentPackage())
...
func TestRegistrationFailureYieldsNoBundle(t *testing.T) {
	...
	issuer := NewCertIssuer(store, repository.NewAgentCertRepository(closedDB(t)), t.TempDir(), "0.0.0.0:8443", testAgentPackage())
