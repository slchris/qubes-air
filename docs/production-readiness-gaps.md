...
| M1-6 升级/回滚契约 | [#14](https://github.com/slchris/qubes-air/pull/14) `e5f7f55` | 四制品 Salt 钉法、协议集合语义（零 flag day）、schema 前向单向（回滚二进制≠回滚数据）、升级顺序与回滚表 |
...
| G-F2 | ~~热路径复杂度：`Tunnel` 262 行 / gocyclo 44；`loadFromEnv` 199 行 / gocyclo 71；`Validate` gocyclo 28。本阶段新增一个配置项即印证了这条判断：`loadFromEnv` 变为 **204 行 / gocyclo 73**（`gocyclo -top` 实测）~~ **已拆分（本次提交）**，3 处 `//nolint:gocyclo[,funlen]` 一并删除、`.golangci.yml` 阈值与豁免规则未动：`Tunnel` 262→36 行 = 生命周期 + `tunnelSession` 上 14 个按帧类型/职责命名的步骤（`startReauthorization` / `newTunnelSession` / `handshake` / `dispatch` / `echoKeepAlive` / `handleRequestHeader` / `openStream` / `handleData` / `writeStreamRequest` / `handleEos` / `dispatchForward` / `handleCallError` / `shutdown` / `closeStreams`，teardown 顺序仍是 cancel→`wg.Wait`→关流，每帧分支的 `break`/`return err` 逐条对齐）；`loadFromEnv` 217→14 行 = 12 个按配置段命名的步骤（`loadServerEnv` / `loadServerRequestLimitsEnv` / `loadStorageEnv` / `loadSecurityEnv` / `loadOrchestratorEnv` / `loadAgentPackageEnv` / `loadOrchestratorTimingEnv` / `loadAgentRenewalEnv` / `loadTransportEnv` / `loadTransportVaultEnv` / `loadTransportTimingEnv` + 原有 `loadQubeSpecBoundsFromEnv`，env 名与"解析失败保留默认"逐字未改）；`Validate` 107→48 行 = 6 个按校验域命名的步骤（原有 `validateAuth` + `validateQrexecAllowlists` / `validateServerTLS` / `validateProductionHardening` / `validateAgentArtifacts` / `validateTransport`，错误串与判定顺序逐条保持）。实测 gocyclo 44→**8** / 76→**1** / 31→**11**，新函数最大 gocyclo **13**（`loadOrchestratorEnv`）、最大 **48 行**（`Validate`）；funlen 拆前 `loadFromEnv` 131 条语句、`Tunnel` 153 条（均 >50），拆后 0 条 | PROJECT_BRIEF §9.2 R-TECH-1/2；复现（共享 cache 必须先清，否则 `relaypb/*.pb.go` 出幽灵告警）：`cd console/backend && golangci-lint cache clean && golangci-lint run --timeout=5m`。拆前该命令 **0 issues 仅因 3 处豁免**；剥掉豁免后同一配置报 `cyclomatic complexity 76/31/44 (> 15)` 与 `too many statements (131/153 > 50)`（funlen 需加 `--uniq-by-line=false`，默认按行去重只留 gocyclo）。拆后同命令 **0 issues**；`gocyclo -over 14 internal/config/config.go internal/transport/grpc/server.go` 空输出。行为保持的旁证：两文件按"非注释行多重集"对比 `origin/main`，差异只剩缩进 / `t.` 前缀 / 函数签名与 `return nil` 收尾；3 处搬运分支各做一次变异红（删 `QUBES_AIR_LOCK_FILE` 查找、反转 stream 端口白名单、删通配 CORS 检查）均由既有测试报错；2 个零覆盖的搬运分支（keepalive 回声、`Frame_Error` 中继）补最小测试 `internal/transport/grpc/tunnel_session_test.go` | 已解除 | ✅ 按业务步骤拆分，未调阈值、未加豁免（`AGENTS.md` 第 51-55 行）。**边界**：全仓另有 3 个**非本次范围**的生产函数高于 15 且各带既有豁免（`agent.validServiceName` 16、`(*Client).recvLoop` 17、`(*AgentProber).Probe` 17，见 G-F3），测试函数由 `.golangci.yml` 的 `_test.go` 窄例外排除。**副作用**：config.go/server.go 行号位移，需同步的引用（机械扫描 merged tree + 逐条比对 old/new 行内容得出，本次按范围约定都不改）：[runtime-defaults](runtime-defaults.md) UD-20（`:971-980`→`:1031-1040`、`:986`→`:1046`）、UD-21（`:1046`→`:1088`）；[upgrade-rollback](upgrade-rollback.md)（`server.go:388-398`→`:489-501`、`:404-406`→`:505-508`）；本文 G-H7（`config.go:1098`→`:1197`）、G-H10（`config.go:1026`，该行是块尾 `}`，需就近重取）；sprint-1 记录 `drift-check.md` TD-1（`server.go:347`→`:345`）与 `partitions/p1-go.md`（`config.go:985` 的注释已随拆分改写）。UD-1h/UD-1i 的引用拆分前就已失准，不列在内 |
| G-F3 | 存量 `//nolint` 无"何时可移除"的退出条件：**本轮实测 20 处 → 17 处**（`git grep -n nolint -- '*.go'`；文档原记"24 处"与当前 revision 不符，按实测值更新），本轮删除的 3 处正是 G-F2 的复杂度/长度豁免。余 17 处分类：`gosec` 10（全部与 `#nosec` 成对，均为已核验的 TLS/常量误报）、`gocritic` 3（`exitAfterDefer`）、`gocyclo` 3、`staticcheck` 1 | 复现：`git grep -n nolint -- '*.go'`（17 处）对照 `git grep -n nolint origin/main -- '*.go'`（20 处）；`cd console/backend && gocyclo -over 15 internal cmd` 给出余下 3 个 gocyclo 豁免函数的实测值——`agent.validServiceName` 16、`(*Client).recvLoop` 17、`(*AgentProber).Probe` 17，**均非本次拆分对象**（豁免理由分别写在 `internal/agent/invoker.go`:108、`internal/transport/grpc/client.go`:386、`internal/service/agentprobe.go`:235） | 技术债（**部分解除**：3/20，余 17 处待办） | 余 17 处每处补退出条件或删除；本轮只交付 3 处（对应 G-F2） |
...
| G-G5 | ~~**`qubes-air-backup` 没有任何发布产物**：`release.yml` 只产出 `qubes-air-console`、console web tarball、agent deb 与 `SHA256SUMS`，Makefile 也没有构建它的 target。于是 M1-5 的 unit 里那个 `ExecStart` 指向一个**只能手工交叉编译**的二进制——而它正是灾难恢复时唯一能读出归档的程序~~ **已修（本次提交，分支 `feat/g5-release-backup-artifact`）**：产物从 4 个变 5 个，`qubes-air-backup` 与 console 同批构建（linux/amd64、`CGO_ENABLED=1`，两者都链 `mattn/go-sqlite3`）、进 `SHA256SUMS`、进 release 的 `files:` 列表；构建的**同一份链接期注入**（`internal/buildinfo` 的三个 `-X`，M2-10 立的方案）现在同时覆盖 console 与 backup——`cmd/qubes-air-backup` 删掉自己的 `-X main.buildVersion`，`--version` 报同样四个字段。构建逻辑按 workflow 头部的规矩从内联抽到 `scripts/build-release-binary.sh`，读回 `--version` 后的**逐字段校验**（`version` 等于 release 版本、`revision`/`build_time` 非 `unknown`、`tree` 为 `clean`/`dirty`，任一缺失即 `FATAL`）对两个制品是同一条代码路径，所以兄弟 flag 拼错不可能只漏掉 backup | 修前：`release.yml` 第 11-14 行（产物自述只有 4 项）、`:106-107`（只 build `./cmd/server`）、`:127-128`（SHA256SUMS 列表）；`grep -rn "cmd/qubes-air-backup" --include='*.yml' --include=Makefile` 无命中。修后：新增步骤 [release.yml](../.github/workflows/release.yml):126-132（Build backup binary，与 `:114-121` 的 console 步骤调用同一个脚本）、`:151`（SHA256SUMS 计入 `qubes-air-backup`）、`:163`（`files:` 列表）、`:10-15`（产物自述）；本地构建 `Makefile`:69-71（`build-backup`，复用 `$(CONSOLE_STAMP)`，与 `:61-63` 的 `build-backend` 同一份 `-X` 拼装）；注入与守卫 `scripts/build-release-binary.sh`:72-75（三个 `-X`）、`:84`（`go build -trimpath`）、`:102-122`（读回 `--version` 并逐字段 `FATAL`）；`--version` 实现 `cmd/qubes-air-backup/main.go`:56-58；测试 `cmd/qubes-air-backup/version_test.go`（真 `go build -ldflags` + 执行 `--version`；无 `-ldflags` 时必须四个 `unknown` 且不含 `dev`；跑 `make build-backup` 后与 `git describe --tags --always --dirty` / `rev-parse HEAD` 逐字比对）；文档 `docs/upgrade-rollback.md` §1 产物表第 4 行与 §3.2 | 已解除（原 **B-阻塞**，对 M1-5 的真机验收是硬依赖） | 已满足：M1-5 的 unit 钉的是发布产物而不是手工编译的二进制，`salt/config.jinja` 的 `backup.binary_sha256` 取 `SHA256SUMS` 里的 `qubes-air-backup` 行（`backup.binary_source` 给 URL），`docs/upgrade-rollback.md` §1 已同步。**未验证**：`release.yml` 从未在 CI 跑过（M1-10），所以制品上的实测签名仍缺；守卫的红/绿证据来自本地对同一脚本的变异（见本次 PR 报告），不是 release 环境 |
...
| G-H8 | ~~console 二进制从不携带构建版本：`appVersion` 是编译期常量 `"0.1.0"`，`release.yml` 与 `Makefile` 都不注入；agent 侧反而有注入~~ **已修并合入 main（PR #22，merge `5ebb687`）**：`appVersion` 常量删除，构建身份改由链接期 `-ldflags -X` 注入新包 `internal/buildinfo`——`version`＝`git describe --tags --always --dirty` 的**原样**输出、`revision`＝`git rev-parse HEAD`（完整 commit）、`build_time`＝链接时刻（RFC 3339 UTC），`tree` 由 `version` 的 `-dirty` 后缀解析成 `clean`/`dirty`/`unknown`。`/health`、`--version`、启动日志、`/status` 报的是**同一份** `buildinfo.Get()`（不是各自取值，避免再造第二个来源）。**未注入的二进制（普通 `go build ./cmd/server`）四个字段一律 `unknown`**，不再有看起来像版本的常量；工作树构建与干净构建用 `-dirty`/`tree=dirty` 区分。**边界**：`internal/transport/grpc.BuildVersion`（握手用的另一处版本面，`frames.go`:40 的 `"dev"`）与 `cmd/qubes-air-backup` 的 `buildVersion` 本轮**未动**（改前者会改 agent/relay 行为，超出 G-H8），因此 console 的 gRPC 握手仍报 `dev` | 修前：`git show origin/main:console/backend/cmd/server/main.go` 第 43 行（`appVersion = "0.1.0"`）、第 66 行（`--version` 打印该常量）、第 70 行（启动日志）、第 1237 行（`/health` 的 `version`）、第 1323 行（`/status`）；`origin/main:.github/workflows/release.yml` 第 106 行（`-ldflags="-s -w"`，不注入）；`origin/main:Makefile` 第 35 行（`go build` 不带 ldflags）；`console/frontend/src/components/Header.svelte`:11（`const version = '0.1.0'`，在 `:29` 渲染成 `v0.1.0`——页眉自带的常量，与构建无关）；对照 `packaging/agent-deb/Dockerfile`:45（agent 侧 `-X main.buildVersion=`）。修后：注入点 `Makefile`:61-63（`build-backend`）、`:85`（`dev`）、[release.yml](../.github/workflows/release.yml):114-121（Build console binary；`:126-132` 的 backup 步骤调同一脚本），其中打包前的**逐字段**校验本体在 `scripts/build-release-binary.sh`:102-122：`version` 必须等于 release 版本、`revision`/`build_time` 不得为 `unknown`、`tree` 必须是 `clean`/`dirty`，任一不满足即以同名 `FATAL: …` 失败——每个字段是独立的 `-X`，只查 `version` 会漏掉兄弟 flag 的拼写错误；取值与回退 `internal/buildinfo/buildinfo.go`:39（`Unstamped`）、`:69`（`TreeUnknown`）、`:95`（`Get`）、`:108`（`String`）；消费点 `cmd/server/main.go`:69（`--version`）、`:73`（启动日志）、`:1030`（读一次）、`:1251`（`healthHandler` 入参）、`:1228`/`:1229`（`healthBody` 的 `build_time`/`tree` 键）、`:1348`（`statusHandler`）；前端类型 `console/frontend/src/lib/types.ts`:232-244，消费方 `console/frontend/src/components/Header.svelte`（页眉从 `/health` 读同一份 `version`；`unknown`/不可达时不显示版本，而不是退回一个常量） | 已解除（待独立复验） | ✅ 变异红（均为实现方自测，命令与输出见 M2-10 条目）：`Get()` 忽略注入值 → `TestGetReportsTheStampedValues` 5/6 子用例 FAIL；`treeState` 恒返回 `TreeClean` → 解析用例 4 个子用例 + Get 用例 2 个 + `TestGetDoesNotClaimCleanForAnUnstampedTree` FAIL；`String()` 不渲染 `revision` → 4 个格式子用例 FAIL；`healthHandler` 把 `version` 写死回 `"0.1.0"` → `/health` 用例 2 个子用例 FAIL；`orUnstamped` 回退成 `"0.1.0"` → `TestPlainGoBuildReportsUnstamped` FAIL 且不再报 `unknown`；**最承重的一条**：只把 `internal/buildinfo` 的 `version` 变量改名、`-X` 保持不变 → 真构建的 `TestBuiltBinaryReportsTheLinkerStamps` FAIL（`version=unknown`，而 `revision`/`build_time` 仍生效），这正是 `-X` 静默失效的探针。release 步骤的校验单独验过（抽出步骤全文执行，见 M2-10 条目）：把 `-X` 的包路径指向不存在的包、以及只把兄弟 flag 拼错（`.revision=`→`.revisions=`、`.buildTime=`→`.buildTimes=`）→ 都是 `exit 1` + `FATAL: console binary carries no … stamp`，而 `version` 本身仍然正确；页眉那边同样变异红：常量回归（3 个用例全红）、`unknown`/catch 分支退回 `'0.1.0'`（各 1 个用例红）。**未验证**：release 制品上的实测（`release.yml` 从未跑通，M1-10）；`docker compose` 的开发镜像（`console/backend/Dockerfile.dev`，镜像与挂载内均无 `.git`）**故意**不注入，仍报 `unknown`；`Makefile:76` 的 `dev` 只核对了 `make -n` 展开与 `build-backend` 同源，未实际长跑 |
...
- [x] **M2-4** 拆分 `Tunnel` / `loadFromEnv` / `Validate`（**已交付并合入 main**：PR #24，`855e337`；gocyclo 44/76/31 → 8/1/11，新函数最大 gocyclo 13、最大 48 行，3 处 `//nolint:gocyclo[,funlen]` 删除且未调阈值；拆前"豁免下 0 issues"与"剥离豁免后 76/31/44 + 131/153 条语句"均已实测，见 G-F2）；为存量 `nolint` 补退出条件（**未交付**：实测 20→17 处，余 17 处仍无退出条件，其中 3 个 gocyclo 豁免的生产函数仍高于阈值 15，见 G-F3）—— G-F2（✅ 已解除）、G-F3（部分解除：余 17 处退出条件仍开放，跟踪在 G-F3 行，不随本项勾选而闭合）
...
# G-G5：qubes-air-backup 的构建入口（修前这条 grep 无命中 → 二进制只能手工编译；
# 现在命中 release.yml 的 backup 步骤与 Makefile 的 build-backup）
cd - >/dev/null
grep -rn "cmd/qubes-air-backup" --include='*.yml' --include=Makefile .
```
