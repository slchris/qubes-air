...
- [ ] **M2-4** 拆分 `Tunnel` / `loadFromEnv` / `Validate`（**已交付，本次提交**：gocyclo 44/76/31 → 8/1/11，新函数最大 gocyclo 13、最大 48 行，3 处 `//nolint:gocyclo[,funlen]` 删除且未调阈值；拆前"豁免下 0 issues"与"剥离豁免后 76/31/44 + 131/153 条语句"均已实测，见 G-F2）；为存量 `nolint` 补退出条件（**未交付**：实测 20→17 处，余 17 处仍无退出条件，其中 3 个 gocyclo 豁免的生产函数仍高于阈值 15，见 G-F3）—— G-F2（✅ 已解除）、G-F3（部分解除，故本项不勾选）
