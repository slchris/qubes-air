...
### 11.1 拓扑：每个 zone 一个网关，qube 不碰 WireGuard

仓库里的现存痕迹一致指向 hub-and-spoke，而不是每台 qube 一条隧道：
...
也就是：**只有网关持有 WG 私钥，且在网关本机生成；qube 完全不参与 WireGuard**，
它们就待在 VPC / 子网里，console 经隧道路由到它们的私网地址。

**这正是 §9.3 那条反对意见的解药。** §9.3 担心「WG 配置得由 cloud-init 送进 guest，
里面含 WG 私钥，于是『不在 cloud-init 里放私钥』变成『换一把私钥放』」——那是在反对
...
- README 里提到 `tailscale/install.sls`，属于另一个仓库且与本方案无关——
  实际连通性是 WireGuard，不要被它带偏。
